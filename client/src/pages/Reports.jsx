...
export default function Reports() {
  ...
  const handlePrint = useReactToPrint({ contentRef: printRef, documentTitle: 'Salary-Statement' });
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        {report && <button onClick={handlePrint} className="btn-primary flex items-center gap-2"><Printer size={16} />Print</button>}
      </div>
      ...
      {report && (
        <div className="card p-6" ref={printRef}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
            ...
            <div><span className="text-[var(--text-secondary)]">Date of Joining:</span> {report.employee?.dateOfJoining || '—'}</div>
            <div><span className="text-[var(--text-secondary)]">Salary Effective From:</span> {report.employee?.salaryEffectiveFrom || '—'}</div>
          </div>
            ...
            <table className="data-table">
              ...
              <tbody>
                ...
                {(report.components || []).map((c, i) => (
                  <tr key={i}><td>{c.name}</td><td className="text-right">{formatCurrency(c.monthlyAmount)}</td><td className="text-right">{formatCurrency(c.yearlyAmount)}</td></tr>
                ))}
                ...
                {(report.deductions || []).map((d, i) => (
                  <tr key={i}><td>{d.name}</td><td className="text-right text-[var(--color-danger)]">{formatCurrency(d.monthlyAmount)}</td><td className="text-right text-[var(--color-danger)]">{formatCurrency(d.yearlyAmount)}</td></tr>
                ))}
                <tr className="border-t-2 border-[var(--border-color)] font-bold bg-[rgba(124,58,237,0.05)]">
                  <td>Net Salary</td>
                  <td className="text-right text-[var(--color-success)]">{formatCurrency(report.netSalary?.monthly)}</td>
                  <td className="text-right text-[var(--color-success)]">{formatCurrency(report.netSalary?.yearly)}</td>
                </tr>
              </tbody>
            </table>
          </div>
